...
export const getModalState = ({
  ...
  state.modals[id] || getInitialModalState({ initialWidth, initialHeight });
...
export const draggableModalReducer = (
  ...
  switch (action.type) {
    case "doubleClick":
      ...
      if (
        state.modals[action.id].width === state.windowSize.width &&
        state.modals[action.id].height === state.windowSize.height
      ) {
        if (
          !prev ||
          (prev.width === state.windowSize.width &&
            prev.height === state.windowSize.height)
        ) {
          prev = {
            x: 0,
            y: 0,
