...
export function branchOptions({
  git_project_id,
  all,
}: pb.git.BranchOptionsRequest) {
  return ajax.get<pb.git.BranchOptionsResponse>(
    `/api/git/projects/${git_project_id}/branch_options?all=${all}`
  );
}
