...
export async function marsConfig({
  git_project_id,
  branch,
}: pb.gitconfig.ShowRequest) {
  return ajax.get<pb.gitconfig.ShowResponse>(
...
export async function toggleGlobalEnabled({
  ...
  return ajax.post<pb.gitconfig.ToggleGlobalStatusResponse>(
...
export async function globalConfig({
  git_project_id,
}: pb.gitconfig.GlobalConfigRequest) {
