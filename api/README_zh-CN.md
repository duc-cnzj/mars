...
流中途错误以 `google.rpc.Status` envelope 返回，还原成 `codes.Error`，与 unary 错误码通用。HTTP/JSON 下拿不到的方法只有三个：client/bidi streaming 的 `Exec`、`StreamCopyToPod`（需要 WebSocket），以及未配 http 注解的 `Project.Apply`——生成器诚实跳过，仅 gRPC SDK 可用。
...
- grpc 包：bufconn 内存 gRPC 测试，覆盖登录/token 前缀/自动刷新/singleflight 去重/拦截器注入/17 个 service 访问器；
- http/transport 包：fake stream/conn 覆盖泛型流工厂与错误传播；
- internal/flight 包：singleflight 去重（同 key 并发只执行一次、其余共享结果、完成后不缓存）全覆盖。
