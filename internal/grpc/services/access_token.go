...
func (a *AccessToken) Lease(ctx context.Context, request *token.LeaseRequest) (*token.LeaseResponse, error) {
	...
	AuditLog(user.Name, types.EventActionType_Update, fmt.Sprintf(`[AccessToken]: 用户 "%s" 续租了 token "%s", 增加了 "%s", 过期时间是 "%s".`, user.Name, at.Token, time.Second*time.Duration(request.ExpireSeconds), at.ExpiredAt.Format("2006-01-02 15:04:05")))
