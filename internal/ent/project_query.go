...
func (pq *ProjectQuery) QueryChangelogs() *ChangelogQuery {
	query := (&ChangelogClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) QueryNamespace() *NamespaceQuery {
	query := (&NamespaceClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withChangelogs; query != nil {
	...
	if query := pq.withNamespace; query != nil {
		if err := pq.loadNamespace(ctx, query, nodes, nil,
...
func (pq *ProjectQuery) loadChangelogs(ctx context.Context, query *ChangelogQuery, nodes []*Project, init func(*Project), assign func(*Project, *Changelog)) error {
...
func (pq *ProjectQuery) loadNamespace(ctx context.Context, query *NamespaceQuery, nodes []*Project, init func(*Project), assign func(*Project, *Namespace)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Project)
	for i := range nodes {
