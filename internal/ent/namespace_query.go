...
func (nq *NamespaceQuery) QueryProjects() *ProjectQuery {
	query := (&ProjectClient{config: nq.config}).Query()
...
func (nq *NamespaceQuery) QueryMembers() *MemberQuery {
	query := (&MemberClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NamespaceQuery) Clone() *NamespaceQuery {
	...
	return &NamespaceQuery{
		...
		withMembers:   nq.withMembers.Clone(),
...
func (nq *NamespaceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Namespace, error) {
	...
	if query := nq.withProjects; query != nil {
		if err := nq.loadProjects(ctx, query, nodes,
	...
	if query := nq.withMembers; query != nil {
		if err := nq.loadMembers(ctx, query, nodes,
...
func (nq *NamespaceQuery) loadProjects(ctx context.Context, query *ProjectQuery, nodes []*Namespace, init func(*Namespace), assign func(*Namespace, *Project)) error {
...
func (nq *NamespaceQuery) loadMembers(ctx context.Context, query *MemberQuery, nodes []*Namespace, init func(*Namespace), assign func(*Namespace, *Member)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Namespace)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
