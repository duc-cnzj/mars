...
func TestNewEndpointSvc(t *testing.T) {
	...
	defer m.Finish()
	svc := NewEndpointSvc(mlog.NewLogger(nil), repo.NewMockEndpointRepo(m))
	assert.NotNil(t, svc)
...
func Test_endpointSvc_InNamespace(t *testing.T) {
	...
	svc := NewEndpointSvc(mlog.NewLogger(nil), endpointRepo)
...
func TestEndpointSvc_InProject_Success(t *testing.T) {
	...
	svc := NewEndpointSvc(mlog.NewLogger(nil), epRepo)
