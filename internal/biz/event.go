...
func (e *eventBiz) AuditLog(action types.EventActionType, username string, msg string) {
	e.eventRepo.AuditLog(action, username, msg)
}

// AuditLogWithChange 记录带变更前后内容的审计日志（透传 repo）。
func (e *eventBiz) AuditLogWithChange(action types.EventActionType, username string, msg string, oldS, newS YamlPrettier) {
	e.eventRepo.AuditLogWithChange(action, username, msg, oldS, newS)
}

// AuditLogWithRequest 记录携带请求体摘要的审计日志（透传 repo）。
func (e *eventBiz) AuditLogWithRequest(action types.EventActionType, username string, msg string, req any) {
	e.eventRepo.AuditLogWithRequest(action, username, msg, req)
}

// FileAuditLog 记录与文件关联的审计日志（透传 repo）。
func (e *eventBiz) FileAuditLog(action types.EventActionType, username string, msg string, fileId int) {
	e.eventRepo.FileAuditLog(action, username, msg, fileId)
}

// FileAuditLogWithDuration 记录与文件关联并携带执行时长的审计日志（透传 repo）。
func (e *eventBiz) FileAuditLogWithDuration(action types.EventActionType, username string, msg string, fileId int, duration time.Duration) {
	e.eventRepo.FileAuditLogWithDuration(action, username, msg, fileId, duration)
}
