...
func (cc *ClusterController) Info(ctx *gin.Context) {
	...
	apiGroups, _ := utils.K8sClientSet().Discovery().ServerGroups()
	metricsAPIAvailable := SupportedMetricsAPIVersionAvailable(apiGroups)
	if !metricsAPIAvailable {
		response.Error(ctx, 500, errors.New("Metrics API not available"))
		return
	}
	var nodes []v1.Node

	// 获取已经使用的 cpu, memory
	nodeList, _ := utils.K8sClient().Client.CoreV1().Nodes().List(context.Background(), metav1.ListOptions{
	...
	list, _ := utils.K8sMetrics().MetricsV1beta1().NodeMetricses().List(context.TODO(), metav1.ListOptions{
		LabelSelector: selector.String(),
	})
